...
namespace VerifyTAPN {
  namespace DiscreteVerification {
    ...
    protected:
      ...
      std::vector<bool> _enabled, _stubborn, _reachable;
      std::vector<std::vector<bool>> _may_reach;
      size_t ecnt = 0;
      bool can_reduce = false;
      light_deque<uint32_t> _unprocessed, _ordering, _waiting;
      ...
      void compute_reachable_actions();
      void add_from_transport_arc(uint32_t parent, uint32_t t1, uint32_t t2);
      void add_from_normal_arc(uint32_t parent, uint32_t t1, uint32_t t2);
      void compute_may_reach_from(uint32_t parent);
      void compute_may_reach();

    public:
      ReducingGenerator(TAPN::TimedArcPetriNet& tapn, AST::Query* query, VerificationOptions options) 
	: Generator(tapn, query), interesting(tapn), _enabled(tapn.getTransitions().size()), _stubborn(tapn.getTransitions().size()), _reachable(tapn.getTransitions().size()), options(options)  { compute_may_reach(); };
